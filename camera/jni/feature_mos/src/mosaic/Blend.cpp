        ...
        float &progress, bool &cancelComputation)
{
    ...
    if (xRightMost <= xLeftMost || yBottomMost <= yTopMost)
    {
        ...
        return BLEND_RET_ERROR_CONSISTENCY;
    }
    ...
    {
        ret = MosaicSizeCheck(LIMIT_SIZE_MULTIPLIER*width/height, LIMIT_HEIGHT_MULTIPLIER*width/height);
    }
...
int Blend::MosaicSizeCheck(float sizeMultiplier, float heightMultiplier) {
   if (Mwidth < width || Mheight < height) {
        return BLEND_RET_ERROR_MOSAIC_SIZE;
    }
