...
int Mosaic::createMosaic(float &progress, bool &cancelComputation)
{
    ...
    switch(ret)
    {
        // notice: consistency check failed
        case Blend::BLEND_RET_ERROR_CONSISTENCY:
            ret = BLEND_RET_ERROR_CONSISTENCY;
            break;
        // notice: mosaic size check failed
        case Blend::BLEND_RET_ERROR_MOSAIC_SIZE:
            ret = BLEND_RET_ERROR_MOSAIC_SIZE;
            break;
