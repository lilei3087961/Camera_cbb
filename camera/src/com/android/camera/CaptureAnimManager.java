...
public class CaptureAnimManager {
    ...
    public boolean drawAnimation(GLCanvas canvas, CameraScreenNail preview,
        ...
        switch (mAnimOrientation) {
            case 0:
                ...
                reviewX = (int)(Math.round(centerX + mHalfGap * scale) * mScanX);
                reviewY = previewY;
                break;
            case 90:
                previewY = Math.round(centerY + mHalfGap * scale);
                previewX = Math.round(centerX - width / 2f);
                reviewY = (int)(Math.round(centerY - height - mHalfGap * scale) * mScanY);
                reviewX = previewX;
                break;
            case 180:
                previewX = Math.round(centerX + width + mHalfGap * scale);
            ...
            case 270:
                ...
                previewX = Math.round(centerX - width / 2f);
                reviewY = (int)(Math.round(centerY + mHalfGap * scale) * mScanY);
                reviewX = previewX;
                break;
        }
        ...
        int cx = reviewX + (int)width / 2;
        int cy = reviewY + (int)height / 2;
        canvas.translate(cx, cy);
        canvas.scale(mScanX, mScanY, 1.0f);
        canvas.translate(-cx, -cy);
    ...
    public void setXYScan(float scanX,float scanY) {
        mScanX = scanX;
        mScanY = scanY;
    }
}
