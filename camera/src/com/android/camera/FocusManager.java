...
public class FocusManager {
    ...
    // Added by zhanghongxing at 2013-06-22
    private static final int SPEED_SHRESHOLD = 150;
    public long mLatestFocusTime;

    public interface Listener {
        public void autoFocus();
        public void silentAutoFocus(); // Added by zhanghongxing at 2013-07-24
    ...
    public void onSilentAutoFocus(boolean focused) {
        if (mState == STATE_FOCUSING_SNAP_ON_FINISH) {
            // Take the picture no matter focus succeeds or fails. No need
            // to play the AF sound if we're about to play the shutter
            // sound.
            ...
            if (focused) {
                mState = STATE_SUCCESS;
                // Do not play the sound in silent auto focus. It does
                // not do a full scan. The focus callback arrives before doSnap
                // so the state is always STATE_FOCUSING.
            } else {
                mState = STATE_FAIL;
    ...
    private void autoFocus() {
        ...
        Log.i("lilei", "111 autoFocus()");
        // BEGIN: Added by zhanghongxing at 2013-06-22
        mLatestFocusTime = System.currentTimeMillis();
    ...
    // BEGIN: Added by zhanghongxing at 2013-07-24
    private void silentAutoFocus() {
        Log.v(TAG, "Start autofocus.");
        mLatestFocusTime = System.currentTimeMillis();
        Log.i("lilei", "111 silentAutoFocus()");
        mListener.silentAutoFocus();
        mState = STATE_FOCUSING;
        // Pause the face view because the driver will keep sending face
        // callbacks after the focus completes.
        if (mFaceView != null) mFaceView.pause();
    ...
    private void cancelAutoFocus() {
        ...
        Log.i("lilei", "222 cancelAutoFocus()");
        //Log.i("lilei", "trace:"+Log.getStackTraceString(new Throwable()));
    ...
    // BEGIN: Added by zhanghongxing at 2013-06-22
    public String getFocusMode (boolean isTapToFocus) {
        if (mOverrideFocusMode != null) return mOverrideFocusMode;
        List<String> supportedFocusModes = mParameters.getSupportedFocusModes();
        String focusMode = null;

        if (mFocusAreaSupported && mFocusArea != null && isTapToFocus) {
            // Always use autofocus in tap-to-focus.
            focusMode = Parameters.FOCUS_MODE_AUTO;
        } else {
            // The default is continuous autofocus.
            focusMode = mPreferences.getString(
                    CameraSettings.KEY_FOCUS_MODE, null);
    ...
    // BEGIN: Added by zhanghongxing at 2013-06-21
    public void onDeviceBecomeStable() {
        ...
        if (mInitialized
                ...
                && mState != STATE_FOCUSING
                && needAutoFocusCall()
                && Util.isTimeout(System.currentTimeMillis(), mLatestFocusTime, 5000)
                && mFocusAreaSupported) {
            // autoFocus();
        	silentAutoFocus();
    ...
    public void onDeviceKeepMoving(double speed) {
        if (mInitialized
                ...
                && needAutoFocusCall()
                && mFocusAreaSupported
                && Util.isTimeout(System.currentTimeMillis(), mLatestFocusTime, 400)) {
            mState = STATE_IDLE;
    ...
    // END:   Added by zhanghongxing at 2013-06-21
}
