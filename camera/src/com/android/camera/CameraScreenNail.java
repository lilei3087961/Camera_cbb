...
public class CameraScreenNail extends SurfaceTextureScreenNail {
    ...
    public interface Listener {
        ...
        int animationRunning(boolean running);
    }
    ...
    public void changeStoragePlace() {
        Log.v(TAG, "changeStoragePlace");
        synchronized (mLock) {
    ...
    public void animateCapture(int animOrientation) {
        synchronized (mLock) {
            // BEGIN: Modified by zhanghongxing at 2013-04-25 for FBD-706
            // GLView's animationRunning() is wrong. Rollback code 2050.
            // mCaptureAnimManager.setOrientation(mListener.animationRunning(true));
    ...
    public void draw(GLCanvas canvas, int x, int y, int width, int height) {
        synchronized (mLock) {
            ...
            if (mAnimState == ANIM_NONE) {
                if(mIsFilmMode) {
                    if(mBeforeFilmModeScanX == 0 || mBeforeFilmModeScanY == 0) {
                        mBeforeFilmModeScanX = mScanX;
                        mBeforeFilmModeScanY = mScanY;
                    }
                    mScanX = 1.0f;
                    mScanY = 1.0f;
                } else {
                    if(mScanX == 0 || mScanY == 0) {
                        mScanX = ((float)getWidth()/width + 0.05f);
                        mScanY = ((float)getHeight()/height + 0.08f);
                    } else if(mBeforeFilmModeScanX > 0 && mBeforeFilmModeScanY > 0){
                        mScanX = mBeforeFilmModeScanX;
                        mScanY = mBeforeFilmModeScanY;
                    }
                }
            ...
            switch (mAnimState) {
                case ANIM_SWITCH_COPY_TEXTURE:
                    //BEGIN: Deleted by zhanghongxing at 2013-04-07 for FBD-429
                    // copyPreviewTexture(canvas);
                    // mSwitchAnimManager.setReviewDrawingSize(width, height);
                    //END:   Deleted by zhanghongxing at 2013-04-07
                ...
                case ANIM_CAPTURE_START:
                    copyPreviewTexture(canvas);
                    mCaptureAnimManager.setXYScan(mScanX,mScanY);
            ...
            if (mAnimState == ANIM_CAPTURE_RUNNING || mAnimState == ANIM_SWITCH_RUNNING) {
                ...
                if (drawn) {
                    ...
                    super.draw(canvas, x, y, width, height);
                    mListener.animationRunning(false);
                }
            }
        } // mLock
    }
    ...
    private void copyPreviewTexture(GLCanvas canvas) {
        ...
        // BEGIN: Modified by zhanghongxing at 2013-04-27 for FBD-929
        Log.i(TAG, "copy preview texture. width = " + width + " . height = " + height + " .");
        try {
            ...
            try {
                ...
                canvas.beginRenderTarget(null);
            }
        }
        // END:   Modified by zhanghongxing at 2013-04-27 for FBD-929
