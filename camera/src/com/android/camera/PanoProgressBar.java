...
class PanoProgressBar extends View {
...
//        canvas.drawRect(mDrawBounds, mBackgroundPaint);
        canvas.drawRoundRect(mDrawBounds, 5, 5, mBackgroundPaint);
        if (mDirection != DIRECTION_NONE) {
        	
        	RectF tmpRect = new RectF();
        	tmpRect.set(mLeftMostProgress, mDrawBounds.top, mRightMostProgress,
        			mDrawBounds.bottom);	
    		// the progress area
    		canvas.drawRoundRect(tmpRect, 5,5,mDoneAreaPaint);
        	...
        	tmpRect.set(l, mDrawBounds.top, r, mDrawBounds.bottom);
        	if(l < mIndicatorWidth) {
        		// the progress area
        		canvas.drawRoundRect(tmpRect, 5,5,mIndicatorPaint);
        	} else {
        		canvas.drawRect(tmpRect,mIndicatorPaint);
        	}
        }
