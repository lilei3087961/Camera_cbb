...
public class IconListPreference extends ListPreference {
    ...
    private int mDefaultLargeIconIds[];
    private ArrayList<String> mUnabledEntry = null ;
	...
	public ArrayList<String>  getUnabledEntry() {
		return mUnabledEntry;
	}
	public IconListPreference(Context context, AttributeSet attrs) {
        ...
        if(getKey().equals(CameraSettings.KEY_VIDEO_QUALITY))
        {
           mDefaultLargeIconIds = mLargeIconIds.clone();
        }
    ...
    public int[] getDefaultLargeIconId() {
        if(mDefaultLargeIconIds != null)
           return mDefaultLargeIconIds;
    ...
    public void filterUnEnabled(List<String> unEnabled){
    	mUnabledEntry = new ArrayList<String>();
    	CharSequence entryValues[] = getEntryValues();
    	for (int i = 0, len = entryValues.length; i<len; i++) {
    		if (unEnabled.indexOf(entryValues[i].toString()) >= 0) {
    	        mUnabledEntry.add(entryValues[i].toString());
   		  }
    	}
    }
}
