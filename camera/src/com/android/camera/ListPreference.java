...
public class ListPreference extends CameraPreference {
    ...
    public ListPreference(Context context, AttributeSet attrs) {
        ...
        if(mKey.equals(CameraSettings.KEY_VIDEO_QUALITY))
        {
            mDefaultEntryValues = mEntryValues.clone();
        }
        a.recycle();
    }
    ...
    public String getEntry() {
        // If can not find current index, set index to the first mEntries.
        int index = findIndexOfValue(getValue());
        if (index == -1) index = 0;
        return mEntries[index].toString();
    }

    public CharSequence[] getDefaultValues() {
        return mDefaultValues;
    }

    public CharSequence[] getDefaultEntryValues() {
        if(mDefaultEntryValues != null)
           return mDefaultEntryValues;
    ...
    public void filterItem(String itemValue) {
        ArrayList<CharSequence> entries = new ArrayList<CharSequence>();
        ArrayList<CharSequence> entryValues = new ArrayList<CharSequence>();
        for (int i = 0, len = mEntryValues.length; i < len; i++) {
            if (!(mEntryValues[i].toString()).equals(itemValue)) {
                entries.add(mEntries[i]);
                entryValues.add(mEntryValues[i]);
            }
        }
    ...
    public void setPrefKey(String newKey) {
    	mKey = newKey;
    }
    public void print() {
