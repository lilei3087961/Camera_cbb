...
public class Thumbnail {
    ...
    private static final String LAST_THUMB_FILENAME_EXTERNAL = "last_thumb_external";
    private static final String LAST_THUMB_FILENAME_INTERNAL = "last_thumb_internal";
    ...
    private Thumbnail(Uri uri, Bitmap bitmap, int orientation) {
        mUri = uri;
        if(mBitmap!=null) mBitmap.recycle(); //Added by xiongzhu at 2013-04-15
    ...
    public void saveLastThumbnailToFile(File filesDir) {
		...
		if(Storage.mIsExternalStorage){
		    file = new File(filesDir, LAST_THUMB_FILENAME_EXTERNAL);
		}else{
		    file= new File(filesDir, LAST_THUMB_FILENAME_INTERNAL);
        }
    ...
    private static Media getLastVideoThumbnail(ContentResolver resolver) {
        ...
        String selection = VideoColumns.BUCKET_ID + '=' + Storage.getBucketId();
        //END:   Modified by xiongzhu at 2013-04-15
    ...
    public static void deleteThumbnailFile(File filesDir) {
		File file = null;
		if(Storage.mIsExternalStorage){
		    file= new File(filesDir, LAST_THUMB_FILENAME_EXTERNAL);
	    }else{
