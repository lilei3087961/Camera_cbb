...
public class AlbumSetBarView extends GLView {
    @SuppressWarnings("unused")
    private static final String TAG = "AlbumSetBarView";
    ...
    public static class SimpleListener implements Listener {
        ...
        @Override public void onSlideLeft() {}
        @Override public void onSlideRight(){}
    }
    ...
    public static interface SlotRenderer {
        ...
        public int renderArrow(GLCanvas canvas, int index, int pass, int width, int height);
    }
    ...
    public AlbumSetBarView(GalleryActivity activity, Spec spec) {
    ...
    private int renderItem(
...

        {
            // draw arrow....
            canvas.save(GLCanvas.SAVE_FLAG_ALPHA | GLCanvas.SAVE_FLAG_MATRIX);
            Rect rect2 = mLayout.getArrowRect(index, mTempRect);
            canvas.translate(rect2.left, rect2.top, 0);
            
            mRenderer.renderArrow(
                    canvas, index, pass, rect2.right-rect2.left, rect2.bottom - rect2.top);
    ...
    // This Spec class is used to specify the size of each slot in the AlbumSetBarView.
    ...
    public class Layout {
        ...
        public Rect getArrowRect(int index, Rect rect) {
            int col, row;
            if (WIDE) {
                col = index / mUnitCount;
                row = index - col * mUnitCount;
            ...
            int x = mWidth - mArrowWidth;
            int y = mVerticalPadding.get() + row * (mSlotHeight + mSlotGap) +
                    (mSlotHeight - mArrowHeight)/2;
            rect.set(x, y, x + mArrowWidth, y + mArrowHeight);
            return rect;
        }
        ...
        private void initLayoutParameters() {
            ...
            if (mSpec.slotWidth != -1) {
                ...
                mArrowWidth = mSpec.arrowWidth;
                mArrowHeight = mSpec.arrowHeight;
            } else {
    ...
    private class MyGestureListener implements GestureDetector.OnGestureListener {
        ...
        public boolean onFling(MotionEvent e1,
            ...
            cancelDown(false);
            if (Math.abs(velocityX) > 60 && Math.abs(velocityY) < Math.abs(velocityX)) {
                if (velocityX > 0) {
