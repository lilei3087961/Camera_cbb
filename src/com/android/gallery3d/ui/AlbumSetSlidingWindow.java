...
public class AlbumSetSlidingWindow implements AlbumSetDataLoader.DataListener {
    ...
    public static class AlbumSetEntry {
        ...
        public BitmapTexture barLabelTexture;
        public Path setPath;
        public String title;
        ...
        private BitmapLoader barLabelLoader;
    }

    public AlbumSetSlidingWindow(GalleryActivity activity,
            AlbumSetDataLoader source, LabelSpec labelSpec, int cacheSize) {
    ...
    private void requestImagesInSlot(int slotIndex) {
        ...
        if (entry.barLabelLoader != null) entry.barLabelLoader.startLoad();
    }
    ...
    private void cancelImagesInSlot(int slotIndex) {
        ...
        if (entry.barLabelLoader != null) entry.barLabelLoader.cancelLoad();
    }
    ...
    private void freeSlotContent(int slotIndex) {
        ...
        if (entry.barLabelLoader != null) entry.barLabelLoader.recycle();
        if (entry.barLabelTexture != null) entry.barLabelTexture.recycle();
    ...
    private void updateAlbumSetEntry(AlbumSetEntry entry, int slotIndex) {
        ...
        if (isLabelChanged(entry, title, totalCount, sourceType)) {
            ...
            if (entry.barLabelLoader != null) {
                entry.barLabelLoader.recycle();
                entry.barLabelLoader = null;
                entry.barLabelTexture = null;
            }
            if (album != null) {
                entry.barLabelLoader = new BarLabelLoader(
                        slotIndex, title, totalCount, sourceType);
            }
        }
    ...
    private void uploadBackgroundTextureInSlot(int index) {
        ...
        if (entry.barLabelTexture != null) {
            mTextureUploader.addBgTexture(entry.barLabelTexture);
        }
    }
    ...
    private void updateTextureUploadQueue() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            ...
            if (entry.barLabelTexture != null) {
                mTextureUploader.addFgTexture(entry.barLabelTexture);
            }
        }
    ...
    private void updateAllImageRequests() {
        ...
        for (int i = mActiveStart, n = mActiveEnd; i < n; ++i) {
            ...
            if (startLoadBitmap(entry.barLabelLoader)) ++mActiveRequestCount;
        }
    ...
    public void onSlotSizeChanged(int width, int height) {
        ...
        for (int i = mContentStart, n = mContentEnd; i < n; ++i) {
            ...
            if (entry.album != null) {
                entry.barLabelLoader = new BarLabelLoader(i,
                        entry.title, entry.totalCount, entry.sourceType);
            }
        }
        updateAllImageRequests();
    ...
    private class BarLabelLoader extends BitmapLoader implements EntryUpdater {
        private final int mSlotIndex;
        private final String mTitle;
        private final int mTotalCount;
        private final int mSourceType;

        public BarLabelLoader(
                int slotIndex, String title, int totalCount, int sourceType) {
            mSlotIndex = slotIndex;
            mTitle = title;
            mTotalCount = totalCount;
        ...
        @Override
        protected Future<Bitmap> submitBitmapTask(FutureListener<Bitmap> l) {
            return mThreadPool.submit(mLabelMaker.requestBarLabel(
                    mTitle, String.valueOf(mTotalCount), mSourceType), l);
        }
