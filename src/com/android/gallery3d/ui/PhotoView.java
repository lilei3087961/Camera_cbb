...
public class PhotoView extends GLView {
    ...
    private StringTexture mFileName;
    private TileImageView mTileView;
    private EdgeView mEdgeView;
    private UndoBarView mUndoBar;
    private BottomNameBarView mBottomName;
    ...
    public PhotoView(GalleryActivity activity) {
        ...
        mBottomName = new BottomNameBarView(context);
        addComponent(mBottomName);
        mBottomName.setVisibility(GLView.VISIBLE);
    ...
    public void notifyImageChange(int index) {
        ...
        invalidate();
        //modify by liuyan to fix the scale
    ...
    protected void onLayout(
        ...
        mBottomName.measure(MeasureSpec.UNSPECIFIED, MeasureSpec.UNSPECIFIED);
        mBottomName.layout(0, h - mBottomName.getMeasuredHeight(), w, h);
    ...
    private void drawFileName(GLCanvas canvas) {
        StringTexture m = mFileName;

        if (mFileName != null) {
            // m.draw(canvas, -m.getWidth() / 2, mWinHeight/2 - 80);
            m.draw(canvas, -m.getWidth() / 2, getHeight() / 2 - 50);
        }
    }

    public void setTitleName(String name, boolean show) {
        mFileName = StringTexture.newInstance(name, DEFAULT_TEXT_SIZE,
                Color.WHITE);
        if(show){
            mBottomName.setVisibility(GLView.VISIBLE);
        }else{
            mBottomName.setVisibility(GLView.INVISIBLE);
        }
        mBottomName.setFileName(mContext, mWinWidth, name);
    }
...
//            if (mModeChanged) return;
    ...
    private void setFilmMode(boolean enabled) {
        ...
        //add by zzw
        mListener.notifyFilmModeStateToScreenNail(enabled);
    ...
    public void pause() {
        ...
        hideUndoBar();
        // Recycle the bitmap resources when pause.
        // If not recycle, memory leak will occur.
        mEdgeView.recycle();
        mUndoBar.recycle();
        mBottomName.recycle();
        if (mVideoPlayIcon instanceof ResourceTexture) {
            ((ResourceTexture)mVideoPlayIcon).recycle();
        }
    }
    ...
    private void showUndoBar(boolean deleteLast) {
        ...
        mBottomName.animateVisibility(GLView.VISIBLE);
    ...
    protected void render(GLCanvas canvas) {
        ...
        // BEGIN: Modified by zhanghongxing at 2013-08-01
        // Remove excess conditions
         boolean full = !mFilmMode && mPictures.get(0).isCamera()
                 && mPositionController.isCenter();
//                 && mPositionController.isAtMinimalScale();
        boolean showAppView = !mFilmMode && mPictures.get(0).isCamera()
                && mPositionController.isCenter();
        // END:   Modified by zhanghongxing at 2013-08-01
        if (full != mFullScreenCamera) {
            mFullScreenCamera = full;
            mListener.onFullScreenChanged(showAppView);
        ...
        renderChild(canvas, mBottomName);
    ...
    private void switchToNextImage() {
        if(mModel.isCamera(mModel.getCurrentIndex())){
    ...
    private boolean switchWithCaptureAnimationLocked(int offset) {
        ...
        if (offset == 1) {
            ...
            //Lock framework orientation to make sure control panel is drawed at the
            //place.
            mListener.lockOrientation();
            switchToFirstImage();
            mPositionController.startCaptureAnimationSlide(1);
