...
public class MenuExecutor {
    ...
    public static void updateMenuOperation(Menu menu, int supported) {
        ...
        boolean supportEditBottom = (supported & MediaObject.SUPPORT_EDIT_BOTTOM) != 0;
...
//        setMenuItemVisibility(menu, R.id.action_edit, supportEdit);
        ...
        //setMenuItemVisibility(menu, R.id.action_edit_bottom, supportEditBottom);
    }
        ...
        switch (action) {
            case R.id.action_select_all:
                if (mSelectionManager.inSelectAllMode()) {
                    if(mSelectionManager.getTotalCount1() == mSelectionManager.getSelectedCount()) {
    ...
    public void startAction(int action, int title, ProgressListener listener,
            boolean waitOnStop, boolean showDialog) {
        //modified by liuyan to get the all path while all selected.
        ArrayList<Path> ids = null;
        if (mSelectionManager.inSelectAllMode()) {
           ids = mSelectionManager.getSelected(true);
        } else {
