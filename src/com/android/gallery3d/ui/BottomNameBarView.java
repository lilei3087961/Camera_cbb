...
public class BottomNameBarView extends GLView {
    private static final String TAG = "BottomNameBarView";
    ...
    // +-+----+----------------+-+--+----+-+------+--+-+
    // 4 16 1 12 32 8 16 4
    public BottomNameBarView(Context context) {
        mContext = context;
        ...
        mPanel = new NinePatchTexture(context, R.drawable.panel_bottom_name_holo);
        ...
        mDeletedText = StringTexture.newInstance(
                ...
                GalleryUtils.dpToPixel(16), mContext.getResources().getColor(R.color.text_bottom_color));
    ...
    public void setFileName(Context context, int winWidth, String fileName){
        mWinWidth = winWidth;
        mDeletedText = StringTexture.newInstance(fileName, GalleryUtils.dpToPixel(16), 
            mContext.getResources().getColor(R.color.text_bottom_color),mWinWidth);
    }
    ...
    public void recycle() {
        ...
        if (mUndoText != null)
            mUndoText.recycle();
        if (mDeletedText != null)
            mDeletedText.recycle();
        if (mUndoIcon != null)
            mUndoIcon.recycle();
    }
    ...
    protected void render(GLCanvas canvas) {
        ...
        int w = getWidth();
        int h = getHeight();
        mPanel.draw(canvas, 0, 0, w , mBarHeight);
       ...
       // canvas.fillRect(x, y, mSeparatorWidth, mBarHeight - mSeparatorTopMargin - mSeparatorBottomMargin, GRAY);
        ...
        if(mContext.getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE){
            mDeletedText.draw(canvas, 1800/2-mDeletedText.mWidth/2, y);
        }else  if (mContext.getResources().getConfiguration().orientation ==Configuration.ORIENTATION_PORTRAIT){
            mDeletedText.draw(canvas, 1080/2-mDeletedText.mWidth/2, y);
        }
