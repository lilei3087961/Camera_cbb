...
import com.android.gallery3d.gif.GifAction;
import com.android.gallery3d.gif.GifDecoder;
import com.android.gallery3d.gif.GifFrame;

public class TileImageView extends GLView implements GifAction{
    public static final int SIZE_UNKNOWN = -1;

    @SuppressWarnings("unused")
    private static final String TAG = "TileImageView";

    private static GifDecoder gifDecoder = null;
    ...
    private MediaItem preItem = null;
    ...
    public TileImageView(GalleryContext context) {
        ...
        mHandler = new MyHandler(mContext, activity.getGLRoot());
    }
    ...
    public void notifyModelInvalidated() {
        ...
        if (mModel == null) {
            ...
            if (mModel instanceof PhotoPage.Model) {
                MediaItem item = ((PhotoPage.Model)mModel).getMediaItem(0);
                if (item != preItem) {
                    preItem = item;
    ...
    private void prepareGif(MediaItem item) {
        if (item != null && (item.getMimeType() != null)
             && item.getMimeType().equals("image/gif")
             && (item.getUri() != null)) {
                //Log.i(TAG, "hzn: " + item.getUri());
                //if (GifDecoderH.getGifFrameNum(mContext.getContentResolver(), item.getUri()) > 1 ) {
                if (hasGifHead(mContext.getContentResolver(), item.getUri())) {
                    Message msg = mHandler.obtainMessage(MSG_SET_GIF_IMAGE, item.getUri());
    ...
    private static final int MSG_SET_GIF_IMAGE = 1;
    ...
    class MyHandler extends SynchronizedHandler {
        ...
        public MyHandler(Context context, GLRoot root) {
            super(root);
        ...
        public void handleMessage(Message message) {
            switch (message.what) {
                case MSG_SET_GIF_IMAGE: {
                    //Log.w(TAG, "hzn handleMessage MSG_SET_GIF_IMAGE 11111111");
                    Uri path = (Uri)message.obj;
                    setGifImage(path);
                    
                    Message m = this.obtainMessage(MSG_UPDATE_FRAME);
                    this.sendMessageDelayed(m, 200);
                    break;
                }
                case MSG_UPDATE_FRAME: {
                    //Log.w(TAG, "hzn handleMessage MSG_UPDATE_FRAME 22222222");
                    GifFrame frame = next();
                    if (frame != null) {
                        Bitmap currentImage = frame.image;
                        BitmapScreenNail newScreenNail = new BitmapScreenNail(currentImage);
                        updateScreenNail(newScreenNail);

                        
                        long sp = frame.delay > 0 ? frame.delay : 50;
                        
                        Message m = this.obtainMessage(MSG_UPDATE_FRAME);
                        this.sendMessageDelayed(m, sp);
                    } else {
                        Message m = this.obtainMessage(MSG_UPDATE_FRAME);
                        this.sendMessageDelayed(m, 10);
                    }
                    break;
                }
        ...
        private boolean setGifImage(Uri uri) {
            boolean ret = true;
            try {
                InputStream is = new BufferedInputStream(
                        mContext.getContentResolver().openInputStream(uri));
                //GifDecoderH.setInputStream(is);
    ...
    private void updateScreenNail(ScreenNail s) {
    ...
    private void stopGifDraw() {
        mHandler.removeMessages(MSG_UPDATE_FRAME);
        mHandler.removeMessages(MSG_SET_GIF_IMAGE);
    }
    
    
    
    
    public boolean hasGifHead(ContentResolver cr, Uri uri) {
        try {
            InputStream in = new BufferedInputStream(cr.openInputStream(uri));
            StringBuffer head = new StringBuffer();
            for (int i=0; i<6; i++) {
                head.append((char)in.read());
            }
            
            if (head.toString().startsWith("GIF")) {
                return true;
            }
