...
public class LocalImage extends LocalMediaItem {
    ...
    public static class LocalImageRequest extends ImageCacheRequest {
        ...
        public Bitmap onDecodeOriginal(JobContext jc, final int type) {
            ...
            if (type == MediaItem.TYPE_MICROTHUMBNAIL) {
                ...
                // try to decode from original image.
                Bitmap bmp = DecodeUtils.decodeThumbnail(jc, mApplication.getAndroidContext(), 
                    mLocalFilePath, options, targetSize, type);
                // if the image is large, decodeThumbnail will return null.
                if (bmp != null) {
                    return bmp;
                }
                
                // try to decode large image get thumbnail image. 
                return DecodeUtils.decodeThumbnailFromLargeBitmap(jc, mLocalFilePath, options);
            } else {
                return DecodeUtils.decodeThumbnail(jc, mApplication.getAndroidContext(), 
                    mLocalFilePath, options, targetSize, type);
            }
        }
    }
    ...
    public static class LocalLargeImageRequest
        ...
        public BitmapRegionDecoder run(JobContext jc) {
        	...
        	Bitmap bitmap = getImageThumbnail(mLocalFilePath);
        	if(bitmap != null){
        		bitmap.recycle();
                return DecodeUtils.createBitmapRegionDecoder(jc, mLocalFilePath, true);
        	}else{
        ...
        public Bitmap getImageThumbnail(String file) {
            BitmapFactory.Options options = new BitmapFactory.Options();
    ...
    public int getSupportedOperations() {
        int operation = SUPPORT_DELETE | SUPPORT_SHARE | SUPPORT_CROP
                | SUPPORT_SETAS | SUPPORT_EDIT | SUPPORT_INFO | SUPPORT_EDIT_BOTTOM;
    ...
    /**
     * This function only can be used in CropImage.
     * Once one image enter croping mode, an new MediaItem whose rotate degree
     *   cannot be changed will be created and the new object always exist until
     *   the original image is deleted.
     * The value of parameter rotate is rotate degree of the original iamge and
    ...
    public MediaDetails getDetails() {
        ...
        // If fail to get width or height value from exif info, then try to get from database.
        String widthValue = (String)details.getDetail(MediaDetails.INDEX_WIDTH);
        String heightValue = (String)details.getDetail(MediaDetails.INDEX_HEIGHT);
        if (widthValue == null || heightValue == null ||
                widthValue.equals("0") || heightValue.equals("0")) {
