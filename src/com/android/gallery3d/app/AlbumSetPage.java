...
public class AlbumSetPage extends ActivityState implements
    ...
    private boolean mShowClusterMenu = true;
    ...
    private void pickAlbum(int slotIndex) {
        ...
        if (mGetVideo) {
            ...
            data.putBoolean(Gallery.KEY_GET_VIDEO, false);
        }
    ...
    private void clearLoadingBit(int loadingBit) {
        ...
        if (mLoadingBits == 0 && mIsActive) {
            // Always show toast when there's no album although we are going to stay on this page.
            if ((mAlbumSetDataAdapter.size() == 0)) {
                if (mSelectionManager.inSelectionMode()) {
                    mSelectionManager.leaveSelectionMode();
                }

                if (!OurEnvironment.getExternalStorageState().equals(OurEnvironment.MEDIA_MOUNTED)
                        && !OurEnvironment.getExternalStorageState().equals(
                                OurEnvironment.MEDIA_MOUNTED_READ_ONLY)) {
                    Toast.makeText((Context) mActivity,
                            R.string.no_external_storage_available, Toast.LENGTH_SHORT).show();
                } else {
                    if(!mOver){
                    /*Toast.makeText((Context) mActivity,
                        ...
                        if(mGetVideo) {
                             Toast.makeText((Context) mActivity,
                                     R.string.empty_video, Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText((Context) mActivity,
                                    R.string.empty_album, Toast.LENGTH_SHORT).show();
                        }
    ...
    private void updateMenuOperations() {
        ...
        MenuItem selectItem = mMenu.findItem(R.id.action_select);
    ...
    protected boolean onCreateActionBar(Menu menu) {
        ...
        if (mGetContent) {
            ...
            if (selectItem != null) {
                // When not in album and the selected action is CLUSTER_BY_ALBUM,
                // we will show "select album", else show "select group".
                // mSelectedAction is from onCreate, can identify cluster action correctly.
    ...
    protected boolean onItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            /*case R.id.action_about:{
            	...
            	AlertDialog.Builder builder = new Builder(mActivity.getAndroidContext());
            	builder.setMessage("Version 2.01_0015(Build E0116 C230)");
            	builder.setTitle(R.string.cbb_gallery_about);
    ...
    private class MyDetailsSource implements DetailsHelper.DetailsSource {
        ...
        public int findIndex(int indexHint) {
            if (mAlbumSetDataAdapter.isActive(indexHint)) {
                MediaSet set = mAlbumSetDataAdapter.getMediaSet(indexHint);
                // When only one media set selected, we find the selected index and update mIndex.
                if (!mSelectionManager.isItemSelected(set.getPath()) &&
                        mSelectionManager.getSelectedCount() == 1) {
                    Path selPath = mSelectionManager.getSelected(false).get(0);
                    int start = mAlbumSetDataAdapter.getActiveStart();
                    int end = mAlbumSetDataAdapter.getActiveEnd();
                    for (int i = start; i < end; i++) {
                        if (selPath == mAlbumSetDataAdapter.getMediaSet(i).getPath()) {
                            indexHint = i;
                            break;
                        }
                    }
