...
import com.android.gallery3d.ui.AlbumSetBarView;
import com.android.gallery3d.ui.AlbumSetBarRenderer;
...
public class AlbumPage extends ActivityState implements GalleryActionBar.ClusterRunner,
    ...
    private boolean mHasBar = true;

    private AlbumSetBarRenderer mAlbumSetBarRenderer;
    private AlbumSetBarView mAlbumSetBar;
    private MediaSet mParentMediaSet;
    private AlbumSetDataLoader mAlbumSetDataAdapter;
    private static final int DATA_CACHE_SIZE = 256;

    private StaticBackground mAlbumSetBarBackground;

    private PhotoFallbackEffect mResumeEffect;
    
    private MenuItem mMenuCamera;
    ...
    private final GLView mRootPane = new GLView() {
        ...
        private static final int mBarWidth = 50;
        ...
        protected void onLayout(
            ...
            int slotWidth = config.slotViewSpec.slotWidth;
            int slotGap = config.slotViewSpec.slotGap;

            if (mHasBar) {
                mAlbumSetBar.setVisibility(GLView.VISIBLE);
                mAlbumSetBarBackground.setVisibility(GLView.VISIBLE);
                if (width < height) {
                    int slotViewWidth = slotWidth * 2 + slotGap * 3 ;
                    
                    if(bottom == 762 && right == 480){
                    	slotViewWidth = slotWidth * 1 + slotGap * 2 +120;
                    }
                    
                    mAlbumSetBar.layout(0, slotViewTop, right - slotViewWidth, bottom);
                    mAlbumSetBarBackground.layout(0, slotViewTop, right - slotViewWidth, bottom);
                    mSlotView.layout(width - slotViewWidth, slotViewTop, slotViewRight, slotViewBottom);
                } else {
                    int slotViewWidth = slotWidth * 4 + slotGap * 5;
                    
                    if(bottom == 480 && right == 800){
                    	slotViewWidth = slotWidth * 3 + slotGap * 4 + 50;
                    }
                ...
                mAlbumSetBar.setVisibility(GLView.INVISIBLE);
                mAlbumSetBarBackground.setVisibility(GLView.INVISIBLE);
    ...
    protected void onCreate(Bundle data, Bundle restoreState) {
    	...
    	String flag = android.os.SystemProperties.get("persist.gallery.showanimation","true");
        ...
        mGetVideo = data.getBoolean(Gallery.KEY_GET_VIDEO, false);
        initializeData(data);
    ...
    protected void onResume() {
        ...
        mActivity.getGalleryActionBar().setDisplayOptions(false, true);
        mActivity.getGalleryActionBar().setHomeButton(true);
    ...
    protected void onPause() {
        ...
        mIsActive = false;
        if (mSelectionManager.inSelectionMode()) {
            mActionModeHandler.dismissSelectionMenu();
            mSelectionManager.leaveSelectionMode();
        }
        mAlbumView.setSlotFilter(null);
    ...
    private void initializeViews() {
        ...
        mSlotView.setListener(new SlotView.SimpleListener() {
            ...
            public void onSlideLeft() {
                AlbumPage.this.onSlideLeft();
            }
            
            @Override
            public void onSlideRight() {
                AlbumPage.this.onSlideRight();
            }
        });
        
        mAlbumSetBarBackground = new StaticBackground(mActivity.getAndroidContext());
        mAlbumSetBarBackground.setImage(R.drawable.bar_background, R.drawable.bar_background);
        mRootPane.addComponent(mAlbumSetBarBackground);        
        
        // albumSetSelectionManager is not used.
        SelectionManager albumSetSelectionManager = new SelectionManager(mActivity, true);
        mAlbumSetBar = new AlbumSetBarView(mActivity, config.albumSetBarSpce);
        mAlbumSetBarRenderer = new AlbumSetBarRenderer(
                mActivity, albumSetSelectionManager, mAlbumSetBar, config.labelSpec);
        mAlbumSetBar.setSlotRenderer(mAlbumSetBarRenderer);
        mAlbumSetBar.setListener(new AlbumSetBarView.SimpleListener() {
            @Override
            public void onSingleTapUp(int slotIndex) {
                if (mAlbumDataAdapter != null) {
                    mAlbumDataAdapter.pause();
                ...
                MediaSet target = mAlbumSetDataAdapter.getMediaSet(slotIndex);
                if (target != null) {
                    mSelectionManager.setSourceMediaSet(target);
                    mMediaSetPath = target.getPath();
                	mMediaSet = mActivity.getDataManager().getMediaSet(mMediaSetPath);
                    mAlbumDataAdapter = new AlbumDataLoader(mActivity, target);
                    ...
                    // update albumset bar
                    mAlbumSetBarRenderer.setHighlightItemPath(target.getPath());
                    
                    
                    mActivity.getGalleryActionBar().setTitle(mMediaSet.getName()+"("+mMediaSet.getMediaItemCount()+")");
                    if (mMenuCamera != null) { 
                    	mMenuCamera.setVisible(true/* 
                                MediaSetUtils.isCameraSource(mMediaSetPath)
                                && GalleryUtils.isCameraAvailable((Activity)mActivity)*/);
                    }
                    
                    mAlbumSetBar.invalidate();
                }
            }
            
        });
        mRootPane.addComponent(mAlbumSetBar);        
    ...
    private void initializeData(Bundle data) {
        ...
        if (mParentMediaSetString == null) {
        	if(mGetVideo){
                ...
                mParentMediaSetString = mActivity.getDataManager().getTopSetPath(DataManager.INCLUDE_LOCAL_VIDEO_ONLY);
        	}else{

                mParentMediaSetString = mActivity.getDataManager().getTopSetPath(DataManager.INCLUDE_ALL);
        	}        	
        }
        
        mParentMediaSet = mActivity.getDataManager().getMediaSet(mParentMediaSetString);
        mAlbumSetDataAdapter = new AlbumSetDataLoader(
                mActivity, mParentMediaSet, DATA_CACHE_SIZE);
        //mAlbumSetDataAdapter.setLoadingListener(new MyLoadingListener());
        mAlbumSetBarRenderer.setModel(mAlbumSetDataAdapter);
        mAlbumSetBarRenderer.setHighlightItemPath(mMediaSetPath);
    }
    ...
    protected boolean onCreateActionBar(Menu menu) {
        ...
        actionBar.setLogo(R.drawable.ic_title_btn_return);
        if (mGetContent) {
            ...
            if (mMediaSet instanceof MtpDevice || mGetVideo) {
    ...
    protected boolean onItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_camera: {
                //Modified by ruijuan.guo to enter Camera in video mode at 2013-12-11 about bug CV-674.
    ...
    private class MyLoadingListener implements LoadingListener {
        ...
        public void onLoadingFinished() {
            ...
            // After load finished, mMediaSet may have changed, to update mActionModeHandler.
            mActionModeHandler.resume();
            mActivity.getGalleryActionBar().setTitle(mMediaSet.getName()+"("+mMediaSet.getMediaItemCount()+")");
        }
    }
    ...
    private class MyDetailsSource implements DetailsHelper.DetailsSource {
        ...
        public int findIndex(int indexHint) {
            if (mAlbumDataAdapter.isActive(indexHint)) {
                MediaItem item = mAlbumDataAdapter.get(indexHint);
                // When only one media item selected, we find the selected index and update mIndex.
                if (!mSelectionManager.isItemSelected(item.getPath()) &&
                        mSelectionManager.getSelectedCount() == 1) {
                    Path selPath = mSelectionManager.getSelected(false).get(0);
                    int start = mAlbumDataAdapter.getActiveStart();
                    int end = mAlbumDataAdapter.getActiveEnd();
                    for (int i = start; i < end; i++) {
                        if (selPath == mAlbumDataAdapter.get(i).getPath()) {
                            indexHint = i;
                            break;
                        }
                    }
                ...
                // If the item isn't active due to some contents deleted by other app,
                // we get the item index by item path in selection manager.
                if (mSelectionManager.getSelectedCount() == 1) {
                    mIndex = mPositionProvider.getItemIndex(mSelectionManager.getSelected(false).get(0));
                }
                else {
